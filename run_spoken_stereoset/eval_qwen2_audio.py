...
def generate_all_permutations(item):
    """
    [NEW] Generate all 6 permutations of 3 options
    """
    raw_options = [
    ...
    for perm_idx, perm in enumerate(permutations):
        # [MODIFIED] Modified prompt to emphasize audio as context
        # Original: "Please listen to the audio and choose the most likely continuation:\n"
        # Modified: emphasize spoken context
...
def prepare_qwen_inputs_full_perm(batch_items, tokenizer, target_sr=16000):
    """
    Prepare Qwen2-Audio vLLM inputs (full permutation version)
    """
    inputs = []
    raw_audios = []
    valid_items = []
...
def main():
    ...
    for model_path in MODELS_TO_RUN:
        ...
        for i in tqdm(range(0, len(items_to_run), chunk_size), desc=f"{model_short_name}"):
            ...
            try:
                prepared_data = prepare_qwen_inputs_full_perm(batch_items, tokenizer, target_sr=16000)
