"""
Evaluation Script for Spoken Stereoset using Qwen2.5-Omni with vLLM
Features:
1. Full Permutation (6 prompts per audio) to eliminate positional bias.
2. Qwen2.5-Omni specific prompt template and processing.
...
# [TODO] Modify to your CSV file path
DATASET_CSV_PATH = "/mnt/data/ycevan/gen_bias/run_spoken_stereoset/spoken_stereoset/metadata/gender_v1.0.csv" 
# [TODO] Audio files root directory
...
def prepare_omni_inputs_full_perm(batch_items, processor, target_sr=16000):
    """
    Prepare Qwen2.5-Omni vLLM inputs (full permutation version)
    """
    inputs = []
    raw_audios = []
    valid_items = []
    ...
    for item, y_padded in zip(valid_items, padded_audios):
        ...
        for instruction, gt_map, perm_idx in perm_results:
            ...
            # [CRITICAL] Qwen2.5-Omni format construction
            # Manually insert audio placeholder, the most stable approach in vLLM
            ...
            try:
                # Use processor.tokenizer to apply the template
...
def parse_args():
    ...
    parser.add_argument("-l", "--limit", type=int, default=None, help="Limit the number of test samples (original data items count)")
    ...
    parser.add_argument("--csv", type=str, default=DATASET_CSV_PATH, help="CSV dataset path")
...
def main():
    ...
    # Simple sorting to keep batch stable
    ...
    for model_path in MODELS_TO_RUN:
        ...
        # 2.1 Load Processor (includes Tokenizer)
        try:
        ...
        try:
            llm = LLM(
                ...
                max_model_len=2048,  # Omni has a longer context window
        ...
        sampling_params = SamplingParams(
            temperature=0.0, 
            top_p=0.75, # Qwen2.5-Omni recommended parameters
            top_k=20,
            max_tokens=128, 
        ...
        for i in tqdm(range(0, len(items_to_run), chunk_size), desc=f"{model_short_name}"):
            ...
            try:
                # Prepare inputs (full permutation version + Omni format)
                prepared_data = prepare_omni_inputs_full_perm(batch_items, processor, target_sr=16000)
