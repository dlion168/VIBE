"""
Evaluation Script for PARADE Dataset using Audio-Flamingo-3 (AF3) with vLLM
Features:
...
def prepare_af3_inputs_parade(batch_items, target_sr=16000):
    """
    Prepare AF3 vLLM inputs (PARADE version)
    Features:
...
def main():
    ...
    if args.exp_id:
        ...
        exp_id = f"parade_af3_{timestamp}"
    ...
    for model_path in MODELS_TO_RUN:
        ...
        for i in tqdm(range(0, len(items_to_run), chunk_size), desc=f"{model_short_name}"):
            ...
            try:
                ...
                inputs_list = prepare_af3_inputs_parade(batch_items, target_sr=16000)
                ...
                with open(output_jsonl, "a", encoding="utf-8") as f:
                    for meta, output_item in zip(inputs_list, outputs):
                        # meta here is directly the "meta" field from elements in inputs_list
                        # But since we zipped inputs_list, meta is the input dict
                        meta_data = meta["meta"]
                        generated_text = output_item.outputs[0].text.strip()
                        
                        record = {
                            "audio_file": os.path.basename(meta_data["path"]), 
                            "full_path": meta_data["path"],
                            "model": model_short_name,
                            "speaker_model": meta_data["speaker_model"], 
                            "domain": meta_data["domain"],
                            "prompt": meta_data["instruction"],
                            "response": generated_text,
                            "option_map": meta_data["option_map"],
                            "label": meta_data["label"],
                            "perm_idx": meta_data["perm_idx"],
                            "task": "PARADE"
                        }
