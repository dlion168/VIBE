"""
Evaluation Script for PARADE Dataset using Qwen2.5-Omni with vLLM
Features:
1. Full Permutation (6 prompts per audio) to eliminate positional bias.
2. Qwen2.5-Omni specific prompt template (<|audio_bos|><|AUDIO|><|audio_eos|>).
...
def prepare_omni_inputs_parade(batch_items, processor, target_sr=16000):
    """
    Prepare Qwen2.5-Omni vLLM inputs (PARADE version)
    """
    inputs = []
    raw_audios = []
    valid_items = []
...
def main():
    ...
    if args.exp_id:
        ...
        exp_id = f"parade_omni_{timestamp}"
    ...
    for model_path in MODELS_TO_RUN:
        ...
        for i in tqdm(range(0, len(items_to_run), chunk_size), desc=f"{model_short_name}"):
            ...
            try:
                # Prepare inputs (SR=16000)
                prepared_data = prepare_omni_inputs_parade(batch_items, processor, target_sr=16000)
        ...
        del llm
        # del processor # Processor usually doesn't take much VRAM, but good practice
        cleanup_vllm()
